...
| SOUND/SSP-SIR | SOUND/SSP-SIR source cue `S1053811917308480` | High-pass filter -> epochs -> pulse interpolation -> baseline -> SOUND -> bad epochs -> ICA for ocular artifacts -> SSP-SIR -> bandpass/notch -> average reference -> downsample -> baseline | The high-pass-before-pulse order is a source-specific/historical protocol description, not the advisor default. For code generation, prefer late analysis filtering; if reproducing this order, warn about filter ringing and require pulse/interpolation-edge QC. |
...
- Downsampling appears early in ARTIST, mid-pipeline in TESA, and late in TMSEEG/Nikulin/SOUND-style summaries. Therefore downsampling should be decided from sampling rate, anti-aliasing, pulse handling, and the time/frequency features needed.
...
- `sound-ssp-sir-enhanced.md` recommends pulse handling before filtering for general use; the SOUND/SSP-SIR table includes high-pass filtering first, so the agent should treat this as a source-specific exception requiring ringing QC.
